...
<body class="container pt-3" style="background-color:#ededed;">
  @php
  $row = \App\Models\User::find(request('id'));
  $school = \App\Models\School::first();
  $classes = \App\Models\Classes::get();
  @endphp
  <h4 class="text-center mb-4">LEMBAR BUKU INDUK PESERTA DIDIK SMP</h4>
  ...
  ">
    <div class="form-group flex-grow-1 pr-3">
      <label for="" style="width:50%;">Nomor Induk Siswa (NISN)</label>
      <input name="nisn" style="border-bottom: 0;" type="text" value="{{isset($row) ? $row->getInformation('personalInformation','nisn'):''}}" required class="form-control form-control-sm" />
    </div>
    <div class="form-group flex-grow-1 pl-3">
      <label for="" style="width:50%;">Nama / Jenis Sekolah</label>
      <input type="text" name="school_name" style="border-bottom: 0;" value="{{$school->name}}" class="form-control form-control-sm" readonly required />
    </div>
  </div>
  <hr />
  <div class="d-flex">
    <div class="flex-grow-1 pr-3">
      <div><h4>A. Keterangan Pribadi</h4></div>
